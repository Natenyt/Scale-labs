...
export async function syncWorkflow(
  ...
  try {
    ...
    if (hasBackendApi()) {
      data = await apiFetch<SyncResponse>(
        `/api/v1/workflows/${record.id}/sync-vapi/`,
        {
          method: "POST",
          json: {
            vapi_payload: compile.payload,
            vapi_workflow_id: record.vapiWorkflowId,
          },
        },
      );
      okHttp = true;
    ...
    if (!okHttp || (data as { error?: string }).error) {
    ...
    const vid = data.vapiWorkflowId ?? (data as { vapi?: { id?: string } }).vapi?.id;
